...
function TestDnd() {
        ...
        setTabList((prev) =>
            prev.map((tab) =>
                tab.id === tabId
                    ? {
                        ...tab,
                        formData: { ...tab.formData, passageRows: rows, passageFirst: 0 },
                    }
                    : tab
            )
        );
    ...
    const handleDragStart = (
        ...
        id,
        originTabId = null,
        originType = "questions"
    ) => {
        const payload = {
            id: String(id),
            originTabId: originTabId == null ? null : String(originTabId),
            originType,
        };
    ...
    const handleDrop = (e, targetTabId) => {
        ...
        try {
            ...
            payload = { id: String(raw), originType: "questions", originTabId: null };
        }
        const idStr = String(payload.id);
        const originType = payload.originType || "questions";
        const originTabId =
            payload.originTabId != null ? String(payload.originTabId) : null;
        setTabList((prevTabs) => {
            let dragged = null;
            ...
            if (originTabId != null) {
                removed = prevTabs.map((tab) => {
                    if (String(tab.id) !== String(originTabId)) return tab;
                    ...
                    if (originType === "questions" || originType === "dropped") {
                        ...
                        if (qi !== -1) {
                            ...
                            return {
                                ...
                                formData: {
                                    ...tab.formData,
                                    questions: q.filter((x) => String(x.id) !== idStr),
                                },
                            };
                        }
                        ...
                        if (di !== -1) {
                            ...
                            return {
                                ...
                                formData: {
                                    ...tab.formData,
                                    droppedQuestions: d.filter((x) => String(x.id) !== idStr),
                                },
                            };
                        }
                    }
                    ...
                    if (originType === "passage" || originType === "droppedPassage") {
                        ...
                        if (pi !== -1) {
                            ...
                            return {
                                ...
                                formData: {
                                    ...tab.formData,
                                    passages: p.filter((x) => String(x.id) !== idStr),
                                },
                            };
                        }
                        ...
                        if (dpi !== -1) {
                            ...
                            return {
                                ...
                                formData: {
                                    ...tab.formData,
                                    droppedPassages: dp.filter((x) => String(x.id) !== idStr),
                                },
                            };
                        }
                    }
            ...
            if (!dragged) {
                ...
                const summary = prevTabs.map((t) => ({
                    tabId: t.id,
                    questionIds: (t.formData?.questions || []).map((x) => String(x.id)),
                    droppedIds: (t.formData?.droppedQuestions || []).map((x) =>
                        String(x.id)
                    ),
                    passageIds: (t.formData?.passages || []).map((x) => String(x.id)),
                    droppedPassageIds: (t.formData?.droppedPassages || []).map((x) =>
                        String(x.id)
                    ),
                }));
                console.log(
                    "Dragged item not found in source arrays:",
                    idStr,
                    "summary:",
                    summary
                );
            ...
            return removed.map((tab) => {
                ...
                if (originType === "dropped" || originType === "questions") {
                    if (originType === "dropped") {
                        ...
                        return {
                            ...
                            formData: {
                                ...tab.formData,
                                droppedQuestions: [...existingDropped, dragged],
                            },
                        };
                    }
                }
                ...
                if (originType === "droppedPassage" || originType === "passage") {
                    if (originType === "droppedPassage") {
                        ...
                        return {
                            ...tab,
                            formData: { ...tab.formData, passages: [...existingP, dragged] },
                        };
                    } else {
                        const existingDroppedP = tab.formData?.droppedPassages || [];
                        if (existingDroppedP.some((x) => String(x.id) === idStr))
                            return tab;
                        return {
                            ...tab,
                            formData: {
                                ...tab.formData,
                                droppedPassages: [...existingDroppedP, dragged],
                            },
                        };
                    }
                }
    ...
    const removeTab = (id) => {
        ...
        if (id == activeTab) {
            ...
            if (tabList.length > 0) {
                newActiveTabId =
                    closedTabIndex > 0 ? tabList[closedTabIndex - 1]?.id : tabList[0]?.id;
            }
    ...
    const handleInputChange = (id, fieldname, val) => {
        setTabList((prevTabs) =>
            prevTabs.map((tab) =>
                tab.id === id
                    ? { ...tab, formData: { ...tab.formData, [fieldname]: val } }
                    : tab
            )
        );
    };
    ...
    const handleOverAllSubmit = async (e) => {
        ...
        if (
            ...
            (!Array.isArray(firstQuestionSource) || firstQuestionSource.length === 0)
        ) {
    ...
    const handleCourseChange = async (id) => {
        console.log(
            "handleCourseChange called with id:",
            id,
            "activeTab:",
            activeTab,
        ...
        try {
            ...
            if (tabId === 0) {
                setTabList((prev) => {
                    ...
                    const updatedFirst = {
                        ...first,
                        formData: { ...first.formData, course: id },
                    };
                    return [updatedFirst];
                });
                setActiveTab(0);
                ...
                setTabList((prev) =>
                    prev.map((tab) =>
                        tab.id === tabId
                            ? { ...tab, formData: { ...tab.formData, course: id } }
                            : tab
                    )
                );
            }
    ...
    const handleCourseData = useCallback(
        async (values, tabId, start = 0, length = 10, author, searchStr = " ") => {
            ...
            try {
                ...
                const totalQ =
                    dataQ?.data?.total_count || dataQ?.data?.count || itemsQ.length;
                setTabList((prev) =>
                    prev.map((tab) =>
                        tab.id === tabId
                            ? {
            ...
            try {
                ...
                const totalP =
                    dataP?.data?.total_count || dataP?.data?.count || itemsP.length;
                setTabList((prev) =>
                    prev.map((tab) =>
                        tab.id === tabId
                            ? {
    ...
    useEffect(() => {
        ...
        setTabList((prev) => {
            ...
            const next = prev.map((tab) => {
                ...
                return {
                    ...tab,
                    formData: { ...tab.formData, questionsFirst: 0, passageFirst: 0 },
                };
            });
            ...
            prev.map((tab) =>
                tab.id === tabId
                    ? {
                        ...
                        formData: {
                            ...tab.formData,
                            advance: { ...(tab.formData?.advance || {}), [field]: value },
                        },
                    }
                    : tab
            )
    ...
    const filteredQuestions = filterBySearch(
        activeTabData?.formData?.questions || [],
        search
    );
    const filteredPassages = filterBySearch(
            ...
            <div className="main__container h-screen">
                ...
                {errorLength > 0 ? (
                    <div
                        className={
                            errorLength !== 0
                                ? "p-2 text-[#26a29a] rounded-xl border bg-white  border-gray-500"
                                : " "
                        }
                    >
                        {errors.title && (
                            <p className="font-semibold text-sm mt-1">{errors.title}</p>
                        )}
              ...
              shadow-md absolute right-0 top-0 transition duration-300 z-40 rounded-t-xl "
                        ...
                        >
                            {tabList.map((tab) => (
                                <Tab
                                    className=" relative bg-none rounded-lg  "
                                                ...
                                                <div className="flex flex-col gap-4">
                                                    ...
                                                    <div className="section__added">
                                                        <p className="text-center ">
                                                            Drag and Drop to re-arrange section below
                                                        </p>
                                                        {tabList.length && (
                                                ...
                                                <>
                                                    ...
                                                    {tab.formData.multiTopics !== "Break" ? (
                                                        <MultiSelect
                                                            ...
                                                            onChange={(e) => {
                                                                const values = Array.isArray(e.value)
                                                                    ? e.value
                                                                    : [];
                                                                handleInputChange(
                                                                    tab.id,
                                                ...
                                                <div className="text-center">
                                                    ...
                                                    {tabList.length > 1 && (
                                                        <div className="mt-3 min-h-20">
                                                            ...
                                                            {tab.formData?.droppedQuestions &&
                                                                tab.formData.droppedQuestions.length > 0 ? (
                                                                        ...
                                                                        >
                                                                            <div className="flex justify-between items-center">
                                                                                ...
                                                                                <div className="text-xs text-gray-500">
                                                                                    <span className="text-[#26a69a] border border-gray-200 rounded-full p-2">
                                                                                        {dq.difficulty} &#9878;{" "}
                                                                                    </span>{" "}
                                                                                    <span className="text-[#26a69a] border border-gray-200 rounded-full p-2">
                                                                                        {dq.evad} &#9878;{" "}
                                                            ...
                                                            <div className="mt-2">
                                                                ...
                                                                {tab.formData?.droppedPassages &&
                                                                    tab.formData.droppedPassages.length > 0 ? (
                                                                    <ul className="space-y-2">
                                                                            ...
                                                                            >
                                                                                <div className="flex justify-between items-center">
                                                                                    ...
                                                                                    <div className="flex items-center gap-2">
                                                                                        <div className="text-xs  border border-gray-600 rounded-lg p-2 text-[#26a69a] ">
                                                                                            {dp.questions?.length || 0} Qs
                                                                                        </div>
                                                                                        <button
                                                    ...
                                                    <div className="inline-flex items-center gap-3 mb-2">
                                                        <div className="inline-flex items-center gap-2 bg-gray-100 px-3 py-1 rounded-full">
                                                            ...
                                                            <span className="bg-[#26a69a] text-white text-sm font-semibold px-2 py-0.5 rounded-full">
                                                                {(tab.formData?.droppedQuestions || []).length}
                                                            </span>
                                                        </div>
                                                        <div className="inline-flex items-center gap-2 bg-gray-100 px-3 py-1 rounded-full">
                                                            <span className="text-sm text-gray-600">
                                                            ...
                                                            <span className="bg-[#26a69a] text-white text-sm font-semibold px-2 py-0.5 rounded-full">
                                                                {(tab.formData?.droppedPassages || []).length}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="question_main">
                                                <div className="flex justify-between items-center bg-white shadow-md rounded-xl p-4">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        >
                                                            Search{" "}
                                                            <p className="bg-[#a1d8d4] rounded-md p-2 ">
                                                                {search}
                                                            </p>
                                                        </label>
                                                        <input
                                                ...
                                                >
                                                    {filteredQuestions.map((q, index) => (
                                                        <li
                                                            ...
                                                            draggable
                                                            onDragStart={(e) =>
                                                                handleDragStart(e, q.id, activeTab, "questions")
                                                            }
                                                ...
                                                <div className="card">
                                                    <Paginator
                                                        ...
                                                        totalRecords={
                                                            activeTabData.formData.questionsTotal || 0
                                                        }
                                                        onPageChange={(e) =>
                                                            onQuestionsPageChange(e, activeTabData.id)
                                                        }
                                                    />
                                                </div>
                                            </div>
                                            ...
                                            <div className="passage_main">
                                                ...
                                                <ul className="space-y-3 mt-4">
                                                    {filteredPassages.map((p, index) => (
                                                        <li
                                                            ...
                                                            draggable
                                                            onDragStart={(e) =>
                                                                handleDragStart(e, p.id, activeTab, "passage")
                                                            }
                                                ...
                                                <div className="card">
                                                    <Paginator
                                                        ...
                                                        totalRecords={
                                                            activeTabData.formData.passageTotal || 0
                                                        }
                                                        onPageChange={(e) =>
                                                            onPassagePageChange(e, activeTabData.id)
                                                        }
                                                    />
                                                </div>
                                            </div>
