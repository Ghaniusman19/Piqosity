...
function TestDnd() {
        ...
        setTabList(prevTabs => {
            ...
            if (!dragged) {
                removed = prevTabs.map(tab => {
                    ...
                    if (idx !== -1) { dragged = d[idx]; return { ...tab, formData: { ...tab.formData, droppedQuestions: d.filter(x => String(x.id) !== idStr) } }; }
    ...
    const handleCourseData = async (values, tabId) => {
        ...
        try {
            ...
            setTabList(prevTabs =>
                prevTabs.map(tab =>
                    tab.id === tabId
                        ? { ...tab, formData: { ...tab.formData, passages: data.data.data } }
                        : tab
                )
            );
        } catch (error) {
    ...
    return (
        <div className="maon__container">
            ...
            <div className='p-2 text-[#26a29a] rounded-xl border  border-gray-500 ' >
                                    ...
                                    <form onSubmit={handleSubmit}
                                        ...
                                        {activeTab == 0 ? (
                                            ...
                                            <>
                                                ...
                                                <span className='text-[#26a69a]' >press <code>ctrl </code> and then select multiple questions </span>
